...
import { Plus, Trash2, ExternalLink, Copy, Check, LogOut, FolderOpen, Eye, X, Layout, AlertTriangle, Loader2, CheckCircle, ShieldAlert, Heart } from 'lucide-react';
...
const AdminDashboard: React.FC<{ user: User }> = ({ user }) => {
  ...
  const copyLink = (link: GDriveLink) => {
    const prefix = link.isPickingMode ? 'p' : 's';
    const url = `${window.location.origin}/${prefix}/${link.shortId}`;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12 relative">
      <AnimatePresence>
        {notification && (
          <motion.div initial={{ opacity: 0, y: -20, x: '-50%' }} animate={{ opacity: 1, y: 0, x: '-50%' }} exit={{ opacity: 0, y: -20, x: '-50%' }} className={cn("fixed top-24 left-1/2 z-[100] px-6 py-3 rounded-full shadow-xl flex items-center gap-3 border", notification.type === 'success' ? "bg-green-500 text-white border-green-600" : "bg-red-500 text-white border-red-600")}>
      ...
      <AnimatePresence>
        {isProcessing && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] bg-black/50 backdrop-blur-sm flex items-center justify-center">
      ...
      <AnimatePresence>
        {deleteId && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white dark:bg-[#1a1a1a] w-full max-w-md p-6 rounded-3xl shadow-2xl border border-slate-200 dark:border-white/10">
              <div className="flex items-center gap-4 mb-6 text-red-500">
                <div className="w-12 h-12 rounded-full bg-red-500/10 flex items-center justify-center"><Trash2 className="w-6 h-6" /></div>
                <h3 className="text-xl font-bold text-slate-900 dark:text-white">Delete Link?</h3>
              </div>
              <p className="text-slate-500 dark:text-white/60 mb-8">Are you sure you want to delete this gallery link? This action cannot be undone.</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-4">
          <div className="p-6 rounded-3xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 sticky top-24 shadow-sm dark:shadow-none">
            <h2 className="text-lg font-semibold mb-6 flex items-center gap-2 text-slate-900 dark:text-white"><Plus className="w-5 h-5" />Create New Link</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-white dark:bg-black border border-slate-200 dark:border-white/10">
                <div className="flex items-center gap-2">
                  <Heart className={cn("w-4 h-4", isPickingMode ? "text-red-500 fill-red-500" : "text-slate-400")} />
                  <span className="text-sm font-medium text-slate-700 dark:text-white">Picking Mode</span>
                </div>
                <button 
                  type="button"
                  ...
                  className={cn(
                    ...
                    isPickingMode ? "bg-red-500" : "bg-slate-200 dark:bg-white/10"
                  )}
                >
                  <div className={cn("absolute top-1 left-1 w-3 h-3 bg-white rounded-full transition-transform duration-200", isPickingMode && "translate-x-5")} />
                </button>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 dark:text-white/30 text-sm">/{isPickingMode ? 'p' : 's'}/</span>
                  <input type="text" value={shortId} onChange={(e) => { setShortId(e.target.value); setIsManualShortId(true); }} placeholder="persona-albume" className="w-full bg-white dark:bg-black border border-slate-200 dark:border-white/10 rounded-xl pl-10 pr-4 py-3 focus:outline-none focus:border-slate-400 dark:focus:border-white/30 transition-colors text-slate-900 dark:text-white text-sm" required />
                </div>
              </div>
              <div>
                ...
                <motion.div key={link.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="group p-5 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20 transition-all flex items-center justify-between shadow-sm dark:shadow-none">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-xl bg-white dark:bg-white/5 flex items-center justify-center text-slate-400 dark:text-white/40 group-hover:text-slate-900 dark:group-hover:text-white transition-colors shadow-sm dark:shadow-none">
                      {link.isPickingMode ? <Heart className="w-6 h-6 text-red-500 fill-red-500" /> : <FolderOpen className="w-6 h-6" />}
                    </div>
                    <div>
                      <h3 className="font-semibold text-slate-900 dark:text-white">{link.title}</h3>
                      <p className="text-sm text-slate-400 dark:text-white/40">/{link.isPickingMode ? 'p' : 's'}/{link.shortId}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => copyLink(link)} className="p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-white/10 text-slate-400 dark:text-white/40 hover:text-slate-900 dark:hover:text-white transition-colors" title="Copy Link">{copiedId === link.id ? <Check className="w-4 h-4 text-green-600" /> : <Copy className="w-4 h-4" />}</button>
                    <a href={`/${link.isPickingMode ? 'p' : 's'}/${link.shortId}`} target="_blank" rel="noopener noreferrer" className="p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-white/10 text-slate-400 dark:text-white/40 hover:text-slate-900 dark:hover:text-white transition-colors"><ExternalLink className="w-4 h-4" /></a>
                    <button onClick={() => setDeleteId(link.id)} className="p-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10 text-slate-400 dark:text-white/40 hover:text-red-600 transition-colors"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </motion.div>
              ))}
            </AnimatePresence>
        ...
        {showPreview && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-white dark:bg-black flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto bg-white dark:bg-[#0a0a0a]">
              <Gallery previewData={{ title: title || "Untitled Gallery" }} isPickingMode={isPickingMode} />
            </div>
          </motion.div>
        )}
      </AnimatePresence>
